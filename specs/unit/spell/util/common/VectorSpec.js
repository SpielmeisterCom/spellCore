...
define(
	...
	function(
		...
		describe( "Vector", function() {
			describe( "non-math functionality", function() {
				it( "should make the vector's component accessible as properties.", function() {
					var x = 1;
					var y = 2;
				...
				it( "should make the vector's components accessible as an array.", function() {
					var x = 1;
					var y = 2;
					
					var vector = new Vector( x, y );
					
					expect( vector.components() ).toEqual( [ x, y ] );
				} );
				
				it( "should be equal to another vector if their components are equal.", function() {
				...
				it( "should create a vector from differenct argument formats.", function() {
					var x = 1;
					var y = 2;
					var vector = new Vector( 1, 2 );
					
					expect( Vector.fromArguments( [ vector ] ) ).toEqual( vector );
					expect( Vector.fromArguments( [ x, y ] ) ).toEqual( vector );
					expect( Vector.fromArguments( [], vector ) ).toEqual( vector );
				} );
			} );
			
			describe( "math functionality", function() {
				it( "should return whether the components are zero.", function() {
					var vector = new Vector( 0, 0 );
					
					expect( vector.isZero() ).toEqual( true );
				} );
				
				it( "should invert itself.", function() {
					var vector = new Vector( 1, 2 );
