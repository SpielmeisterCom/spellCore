...
define(
	...
	function() {
		...
		var each = function(list, iterator, context) {
			if (list == null) return;
			if (nativeForEach && list.forEach === nativeForEach) {
				list.forEach(iterator, context);
			} else if (list.length === +list.length) {
				for (var i = 0, l = list.length; i < l; i++) {
					if (i in list && iterator.call(context, list[i], i, list) === breaker) return;
				}
			} else {
				for (var key in list) {
					if (_.has(list, key)) {
						if (iterator.call(context, list[key], key, list) === breaker) return;
					}
				}
			}
		};
		...
		_.map = function(list, iterator, context) {
			...
			if (nativeMap && list.map === nativeMap) return list.map(iterator, context);
			each(list, function(value, index, list) {
				results[results.length] = iterator.call(context, value, index, list);
			});
			if (list.length === +list.length) results.length = list.length;
			return results;
		};
